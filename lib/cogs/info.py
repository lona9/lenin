...
class Info(Cog):
  ...
  async def userinfo(self, ctx, target: Optional[Member]):
    ...
    fields = [("Name", str(target), True),
              ...
              ("Activity", f"{str(target.activity.type).split('.')[-1].title() if target.activity else 'N/A'} {target.activity.name if target.activity else 'N/A'}", True),
